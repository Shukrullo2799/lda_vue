...
async function fetchUrl() {
  const res = await axios.get(
    "https://opensheet.elk.sh/1XOI_hiwB8n9Wy-PZKwGvFUpKKPQSgYCaqpEgBbszsNk/lda",
  )
  ...
  for (var i = 0; i < locales.length; i++) {
    ...
    for (var j = 0; j < data.length; j++) {
      ...
      if (key) {
        ...
        if (locale === "oz") {
          setNestedProperty(localeData["uz-Cyrl"], key, parseValue(translation), "cyrl")
        }
  ...
  Object.entries(localeData).forEach(([key, value]) => {
    const way = key === "oz" ? "uz-Latn" : key
    fs.writeFile(`./src/app/i18n/locales/${way}.json`, JSON.stringify(value, null, 2), (err) => {})
  })
